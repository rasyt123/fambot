...
bool Chess::Knight::GenerateMoves(std::vector<std::vector<char>>& underboard, std::vector<std::vector<Pieces>>& thepieces, std::string color) {
    ...
    {
        if (InBounds(horsemove.first, horsemove.second, underboard) and underboard[horsemove.first][horsemove.second] != ' '
                       and thepieces[horsemove.first][horsemove.second].getcolor() == color)
        {
            protectingsquares.emplace_back(horsemove.first, horsemove.second);
        }
