...
int Chess::Rook::getstaredownkingy() {
    return staredownkingy;
}


int Chess::Rook::getstaredownkingx() {
...
bool Chess::Rook::gettop(std::vector<std::vector<char>>& underboard, std::vector<std::vector<Pieces>>& thepieces, std::string color, int posy, int posx) {
    for (int x = 1; x < underboard.size(); x++)
    {
        ...
        {
            break;
        } else if (underboard[posy - x][posx] != ' ' and thepieces[posy - x][posx].getcolor() == color)
        {
            protectingsquares.emplace_back(std::make_pair(posy - x, posx));
            break;
        } else if (underboard[posy - x][posx] == 'A' and thepieces[posy - x][posx].getcolor() != color)
        {
            staredownkingy = posy - (x - 1);
            staredownkingx = posx;
            starekingdown = true;
            return true;
...
bool Chess::Rook::getadjacentleft(std::vector<std::vector<char>>& underboard, std::vector<std::vector<Pieces>>& thepieces, std::string color, int posy, int posx) {
    for (int x = 1; x < underboard.size(); x++)
    {
        ...
        {
            break;
        } else if (underboard[posy][posx - x] != ' ' and thepieces[posy][posx - x].getcolor() == color)
        {
            protectingsquares.emplace_back(std::make_pair(posy, posx - x));
            break;
        } else if (underboard[posy][posx - x] == 'A' and thepieces[posy][posx - x].getcolor() != color)
        {
            staredownkingy = posy;
            staredownkingx = posx - (x - 1);
            starekingdown = true;
            return true;
